...
export const createRuntimeExecutors = (
  ...
  // Per-operation tool call counter for hook callIndex
  ...
  return {
    ...
    call_llm: async (instruction, state) => {
      ...
      const toolDiscoveryConfig = buildToolDiscoveryConfig(
        operationToolSet,
        ...
        if (agentConfig) {
          ...
          if (isOnboardingAgent && !alreadyHasOnboardingContext && ctx.serverDB && ctx.userId) {
            try {
              ...
              log(
                'Built onboarding context for agent %s, phase: %s',
                agentId,
    ...
    compress_context: async (instruction, state) => {
      ...
      try {
        ...
        const latestAssistantMessage = dbMessages.findLast(
    ...
    call_tool: async (instruction, state) => {
      ...
      try {
        ...
        // Parse arguments for hook event
        ...
        if (mockResult) {
          // Mock path: skip real execution entirely
    ...
    call_tools_batch: async (instruction, state) => {
      ...
      for (const tp of toolsCalling) {
        const src =
          state.operationToolSet?.sourceMap?.[tp.identifier] ??
          state.toolSourceMap?.[tp.identifier];
        if (src === 'client') {
          clientTools.push(tp);
        } else {
          serverTools.push(tp);
